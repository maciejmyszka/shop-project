...
const CategoryForm = ({
  ...
  const editedId = categories.filter(
    (category: SingleCategory) => category.name === choosedCategory
  )[0].id;
  ...
  const editCategory = (e: Event) => {
    ...
    api
      .put(`/ajax/219/product_categories/${editedId}`, editedElement)
  ...
  const deleteCategory = (e: Event) => {
    e.preventDefault();
    api
      .delete(`/ajax/219/product_categories/${editedId}`)
      .then((response: AxiosResponse<any>) => setStatus(response.status));
    setNewValue("");
    setChoosedCategory("");
  };
    ...
    <Form>
      <Form.Group className="mb-3" controlId="formBasicEmail">
        ...
        <Form.Control
          ...
          value={newValue}
          onChange={(e: ChangeEvent<any>) => setNewValue(e.target.value)}
        />
      </Form.Group>
      <div className="buttons-wrapper">
        <Button
          variant="success"
          onClick={(e: MouseEvent<HTMLElement>) => editCategory(e as any)}
        >
          Potwierdź
        </Button>
        <Button
          variant="danger"
          onClick={(e: MouseEvent<HTMLElement>) => deleteCategory(e as any)}
        >
          Usuń
        </Button>
      </div>
