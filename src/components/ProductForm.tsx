...
const ProductForm = ({
  ...
  const editedProductId = products.filter(
    (product: SingleProduct) => product.name === choosedProduct
  )[0].id;
  ...
  const editProduct = (e: Event) => {
    ...
    api
      .put(`/ajax/219/products/${editedProductId}`, newEl)
  ...
  const deleteProduct = (e: Event) => {
    e.preventDefault();
    api
      .delete(`/ajax/219/products/${editedProductId}`)
      .then((response: AxiosResponse<any>) => setStatus(response.status));
    setChoosedProduct("");
  };
  ...
  return (
    <Form>
      ...
      >
        <option disabled hidden selected value="categories">
          Kategorie
        </option>
      ...
      <div className="buttons-wrapper">
        <Button
          variant="success"
          onClick={(e: MouseEvent<HTMLElement>) => editProduct(e as any)}
        >
          Potwierdź
        </Button>
        <Button
          variant="danger"
          onClick={(e: MouseEvent<HTMLElement>) => deleteProduct(e as any)}
        >
          Usuń
        </Button>
      </div>
